...
const getAllBTPs = async (req, res) => {
    const commerce = await BTP.find({type: 'commerce'})
    const art = await BTP.find({type: 'art'})
    const informatique = await BTP.find({type: 'informatique'})
    return res.render("pages/diplome/btp",{commerce, art,informatique})
}
